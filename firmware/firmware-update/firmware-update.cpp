...
{
	firmwareUpdate.Render(time);
}

void update(uint32_t time)
{
	firmwareUpdate.Update(time);
}
...
void FirmwareUpdate::Init()
{
	blit::set_screen_mode(ScreenMode::hires);

	// register FIRM
	g_commandStream.AddCommandHandler(CDCCommandHandler::CDCFourCCMake<'F', 'I', 'R', 'M'>::value, this);

	m_uCurrentFile = persist.selected_menu_item;
}


// FSInit() Read a page worth of *.FRM filenames from the SDCard
void FirmwareUpdate::FSInit(void)
{
...
// Flash(): Flash a file from the SDCard to internal flash
bool FirmwareUpdate::Flash(const char *pszFilename)
{
	bool bResult = false;
...
void FirmwareUpdate::Render(uint32_t time)
{
	switch(m_state)
	{
		case stFlashFile:
...
void FirmwareUpdate::RenderMassStorage(uint32_t time)
{
...
void FirmwareUpdate::RenderFlashCDC(uint32_t time)
{
	screen.pen = Pen(0,0,0);
	screen.rectangle(Rect(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT));
	screen.pen = Pen(255, 255, 255);
...
void FirmwareUpdate::RenderFlashFile(uint32_t time)
{
...
void FirmwareUpdate::Update(uint32_t time)
{
}
...
bool FirmwareUpdate::StreamInit(CDCFourCC uCommand)
{
	//printf("streamInit()\n\r");
	m_fPercent = 0.0f;
	...
	switch(uCommand)
	{
		case CDCCommandHandler::CDCFourCCMake<'F', 'I', 'R', 'M'>::value:
			m_state = stFlashCDC;
...
// FlashData() Flash data to the internal flash
// Note: currently qspi_write_buffer only works for sizes of 256 max
bool FirmwareUpdate::FlashData(uint32_t uOffset, uint8_t *pBuffer, uint32_t uLen)
{
	bool bResult = false;
...
CDCCommandHandler::StreamResult FirmwareUpdate::StreamData(CDCDataStream &dataStream)
{
